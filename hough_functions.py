...
References:
  P. N. Swarztrauber and A. Kasahara (1985), The vector harmonic analysis of Laplace's tidal equations,
                                             SIAM J. SCI. STAT. COMPUT. Vol. 6, No.2. 464--491

  A. Kasahara (1976), Normal modes of ultralong waves in the atmosphere,
                      Monthly Weather Review. Vol. 104, No.6. 669--690

  Y. Shigehisa (1983), Normal Modes of the Shallow Water Equations for Zonal Wavenumber Zero,
                       J. Meteor. Soc. Japan. Vol. 61, No. 4. 479--493

  A. Kasahara (1978), Further Studies on a Spectral Model of the Global Barotropic Primitive Equations
                      with Hough Harmonic Expansions,
                      J. Atmos. Sci. Vol. 35, No.11. 2043--2051
'''
...
def hvf(hk,M=42,nLR=40,nLG=20,NEH=15,dlat=1.5,save=False):
  '''
  ...
  NEH  - Number of vertical normal modes retained (Number of Equivalent Heights)
  ...
  uP0_n = np.zeros((2*N,np.shape(x)[0]),dtype=dType) # unnormalized
  P0_n  = np.zeros((2*N,np.shape(x)[0]),dtype=dType) # normalized
  ...
  #for n in np.arange(0,N):
  ## Looping over the latitudes in x
    #for z in np.arange(0,np.shape(x)[0]):
      #[P0n,dP0n_dz] = sp.special.lpmn(m, n, x[z])
      #uP0_n[n,z]    = P0n[m,n];
  ...
  # Deffining the array for the Associated Legendre Functions (Unnormalized and normalized)
  uP1_n = np.zeros((2*N,np.shape(x)[0]),dtype=dType)   # unnormalized
  P1_n  = np.zeros((2*N,np.shape(x)[0]),dtype=dType)   # normalized
  ...
  for m in range(1,M+1):
    ...
    print '  - selecting freqs and coeffs'
    # Selecting the frequencies and the coefficients for the vector harmonic expansion.
    ...
    # Deffining the array for the Associated Legendre functions (Unnormalized and normalized)
    # The Unnormalized
    uPm_n    = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    uPmm1_n  = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    uPmM1_n  = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    uPmm1_n1 = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    uPmM1_n1 = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    # The Normalized
    Pm_n    = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    Pmm1_n  = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    PmM1_n  = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    Pmm1_n1 = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    PmM1_n1 = np.zeros((2*N,np.shape(x)[0]),dtype=dType)


    print '  - Associated Legendre Functions'
    # Computing first two Associated Legendre Functions
    ...
    dPm_n_dLat = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    ...
    mPm_n_cosLat = np.zeros((2*N,np.shape(x)[0]),dtype=dType)
    for n in range(m,2*N+m):
    ...
    print '  - spherical vector harmonics'
    ...
    y1m_n = np.zeros((3,2*N,np.shape(x)[0]),dtype=cType)
    y2m_n = np.zeros((3,2*N,np.shape(x)[0]),dtype=cType)
    y3m_n = np.zeros((3,2*N,np.shape(x)[0]),dtype=cType)
  ...
  out=dict(HOUGH_UVZ=HOUGH_UVZ,HOUGH_0_UVZ=HOUGH_0_UVZ,WEST_G_sy=WEST_G_sy,WEST_G_asy=WEST_G_asy,WEST_R_asy=WEST_R_asy,WEST_R_sy=WEST_R_sy,
           EAST_G_sy=EAST_G_sy,EAST_G_asy=EAST_G_asy,WEST_G_0_sy=WEST_G_0_sy,WEST_G_0_asy=WEST_G_0_asy,EAST_G_0_sy=EAST_G_0_sy,
           EAST_G_0_asy=EAST_G_0_asy,WEST_R_0_asy=WEST_R_0_asy,WEST_R_0_sy=WEST_R_0_sy)

  if save:
    if isinstance(save,basestring): fsave=save
    else: fsave='out_hvf_M%d_nLR%d_nLG%d_NEG%d_dlat%s.npz'%(M,nLR,nLG,NEH,dlat)
